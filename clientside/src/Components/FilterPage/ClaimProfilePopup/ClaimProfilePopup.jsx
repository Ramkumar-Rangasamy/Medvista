...
const ClaimProfilePopup = ({ doctorId, handleCloseClaimPopup }) => {
  ...
  const handleSubmit = async (e) => {
    ...
    formData.append("doctorId", doctorId); // Include doctorId
    formData.append("email", email);
    formData.append("document", fileInputRef.current.files[0]);

    try {
      const response = await axios.post(
        `${process.env.REACT_APP_BASE_URL}/doctor/claim-profile`,
        formData,
        {
          headers: {
